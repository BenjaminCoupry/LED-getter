...
def preprocess(ps_images_paths, sliced=slice(None), meshroom_project=None, aligned_image_path=None,
    ...
    pose = loading.load_pose(pose_path if pose_path else (meshroom_project if meshroom_project else geometry_path), aligned_image_path if aligned_image_path else ps_images_paths) if pose_path or meshroom_project or geometry_path else None
    pixelmap = loading.get_pixelmap(pose if (pose and pose['width'] is not None and pose['height'] is not None) else ps_images_paths[0])[sliced]
    geometric_mask, normalmap, pointmap, raycaster  = loading.load_geometry(geometry_path if geometry_path else meshroom_project, pixelmap, pose, flip_mesh=flip_mesh, batch_size = 1000, apply_undisto=apply_geometry_images_undisto)
    geom_images, undisto_mask, (_, n_im, n_c) = loading.load_images((ps_images_paths + [black_image_path]) if black_image_path else ps_images_paths, pixelmap[geometric_mask], pose, batch_size = 1000, apply_undisto=apply_images_undisto)
